...
import (
	...
	"github.com/manhrev/runtracking/backend/activity/pkg/ent/migrate"
...
// A call to `Use(f, g, h)` equals to `entactivity.Hooks(f(g(h())))`.
func (c *ActivityClient) Use(hooks ...Hook) {
	c.hooks.Activity = append(c.hooks.Activity, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `entactivity.Intercept(f(g(h())))`.
...
func (c *ActivityClient) DeleteOneID(id int64) *ActivityDeleteOne {
	builder := c.Delete().Where(entactivity.ID(id))
...
func (c *ActivityClient) Get(ctx context.Context, id int64) (*Activity, error) {
	return c.Query().Where(entactivity.ID(id)).Only(ctx)
}
