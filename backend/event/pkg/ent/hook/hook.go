...
// function as MemberProgress mutator.
type MemberProgressFunc func(context.Context, *ent.MemberProgressMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f MemberProgressFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.MemberProgressMutation); ok {
		return f(ctx, mv)
	}
...
// Mutate calls f(ctx, m).
func (f SubEventFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.SubEventMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.SubEventMutation", m)
}
