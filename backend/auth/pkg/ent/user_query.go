...
func (uq *UserQuery) QueryUserSetting() *UserSettingQuery {
	query := &UserSettingQuery{config: uq.config}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withUserSetting: uq.withUserSetting.Clone(),
...
func (uq *UserQuery) WithUserSetting(opts ...func(*UserSettingQuery)) *UserQuery {
	query := &UserSettingQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserSetting = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withUserSetting; query != nil {
		if err := uq.loadUserSetting(ctx, query, nodes, nil,
			func(n *User, e *UserSetting) { n.Edges.UserSetting = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadUserSetting(ctx context.Context, query *UserSettingQuery, nodes []*User, init func(*User), assign func(*User, *UserSetting)) error {
	...
	query.Where(predicate.UserSetting(func(s *sql.Selector) {
		s.Where(sql.InValues(user.UserSettingColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_user_setting
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_user_setting" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_user_setting" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
