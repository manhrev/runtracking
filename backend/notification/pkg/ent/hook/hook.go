...
// The NotificationUserFunc type is an adapter to allow the use of ordinary
// function as NotificationUser mutator.
type NotificationUserFunc func(context.Context, *ent.NotificationUserMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f NotificationUserFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.NotificationUserMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.NotificationUserMutation", m)
}
