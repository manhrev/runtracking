...
export default function ChallengeEdit({
  ...
  const [challengeInfo, setChallengeInfo] = useState<ChallengeInfo.AsObject>({
    ...
    type:
      route.params.challengeInfo?.type ||
      ActivityType.ACTIVITY_TYPE_UNSPECIFIED,
    status:
      route.params.challengeInfo?.status || RuleStatus.RULE_STATUS_UNSPECIFIED,
  ...
  const getOldGoal = (rule: Rule) => {
    ...
    return rule == Rule.RULE_TOTAL_DISTANCE
      ? 1
  ...
  const [dropdownSelected, setDropdownSelected] = useState<string[]>(
    route.params.challengeInfo
      ? route.params.challengeInfo?.challengerulesList.map((item) =>
          item.rule.toString()
        )
      : []
  )
    ...
    for (let i = 0; i < dropdownSelected.length; i++) {
      ...
      const goal =
        rule == Rule.RULE_TOTAL_DISTANCE
          ...
          ? goals.time
          : goals.calories
      if (oldDropdown.includes(dropdownSelected[i])) {
        // already in old rule list
        oldRuleList.push({
