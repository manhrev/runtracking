...
export default function ChallengeList({
  ...
  const addOrRemoveFromDeleteList = (id: number) => {
    if (deleteListId.includes(id)) {
      ...
      if (deleteListId.length + 1 === challengeList.length) setSelectedAll(true) // if all selected
    }
  }
  ...
  const deleteConfirmed = () => {
    ...
    deleteListId.forEach(async (id) => {
      ...
      const { error } = await dispatch(
        deleteChallengeThunk(deleteInfo)
      ).unwrap()
      if (error) {
        failedCount++
      }
                  ...
                  actions={[
                    {
                      ...
                      onPress: () =>
                        navigation.navigate('ChallengeAdd', {
                          groupId: route.params.groupId,
                          reloadListFunc: fetchChallengeList,
                        }),
                    },
                    {
                      // select or unselect all
                ...
                <ChallengeItem
                  ...
                  goToChallengeDetail={() =>
                    navigation.navigate('ChallengeDetail', {
                      ...
                      canEdit:
                        route.params.isLeader &&
                        challenge.status === RuleStatus.RULE_STATUS_COMING_SOON,
